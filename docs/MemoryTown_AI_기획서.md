...
7. 최종 리포트: 특정 Agent 시점의 제출용 Markdown 리포트 생성
8. 제출 체크리스트: 과제 조건 충족 여부 확인
...
- Mock 모드는 제출 전 데모에는 좋지만 실제 AI 품질 검증은 Real AI 모드 또는 Ollama 로컬 모드가 필요하다.
...
Real AI 모드는 `.env` 파일 또는 환경변수에 `OPENAI_API_KEY`를 설정한 뒤 앱 사이드바에서 선택한다.

## 과제 요구사항 충족표

| 요구사항 | 구현 위치 | 충족 여부 |
| --- | --- | --- |
| AI를 활용한 스타트업 서비스 기획 | docs/MemoryTown_AI_기획서.md | 충족 |
| Streamlit 웹앱 구현 | app.py | 충족 |
| OpenAI API 사용 가능 | memorytown/llm_client.py | 충족 |
| Ollama 로컬 LLM 사용 가능 | memorytown/llm_client.py | 추가 충족 |
| API Key 하드코딩 금지 | .env.example, 환경변수 사용 | 충족 |
| Mock 모드 지원 | MockLLMClient | 충족 |
| Agent 기본 구조 | memorytown/models.py | 충족 |
| Memory 라운드별 Fact 저장 | memorytown/simulation.py | 충족 |
| Relation Map Reflection | memorytown/models.py, simulation.py | 충족 |
| 수동 대화 라운드 | app.py, simulation.py | 충족 |
| Fact 추출 및 저장 | llm_client.py, simulation.py | 충족 |
| Daily Plan 생성 | llm_client.py, simulation.py | 충족 |
| 같은 장소 Agent 자동 대화 | simulation.py | 충족 |
| 이틀 후 리포트 저장 | app.py, report.py | 충족 |
| 테스트 포함 | tests/ | 충족 |
| 제출용 zip 생성 | scripts/make_submission_zip.py | 충족 |
